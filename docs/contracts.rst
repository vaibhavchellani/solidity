...
When a contract is created, its constructor (a function declared with the ``constructor_`` keyword) executes any code in the function once.
...
A constructor is an optional function declared with the ``constructor`` keyword which is executed upon contract creation, and designed to run contract initialisation code. It is always generates code that initialises any state variables you declare to their specified default value if you initialise them inline, or zero if you do not.
